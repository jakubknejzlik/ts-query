...
Use `Q.null()` to add NULL as a field value. This is preferred over `Q.raw("NULL")` for clarity:
...
**Important:** You must wrap the subquery with `Q.table()` when using it in a join. Passing a subquery directly to `leftJoin()` or other join methods will result in an error.
...
Use `Q.values()` to create a SELECT query from static data. It generates `SELECT ... UNION ALL SELECT ...` which works across all SQL engines.
...
<QueryPreview
  code={`
Q.createTableAs('new_table', Q.select().from('users').where(Cond.equal('active', true)));
`}
/>

### CREATE OR REPLACE TABLE

Use `createOrReplaceTableAs` to replace the table if it already exists:

<QueryPreview
  code={`
Q.createOrReplaceTableAs('new_table', Q.select().from('users'));
`}
/>

### CTAS with Values

Combine `createTableAs` with `Q.values()` to create a table from static data:

<QueryPreview
  code={`
Q.createTableAs('seed_data', Q.values([
  { id: 1, name: 'Alice' },
  { id: 2, name: 'Bob' },
]));
`}
...
<QueryPreview
  code={`
Q.createViewAs('active_users', Q.select().from('users').where(Cond.equal('active', true)));
`}
/>
...
<QueryPreview
  code={`
Q.createOrReplaceViewAs('active_users', Q.select().from('users').where(Cond.equal('active', true)));
`}
/>
