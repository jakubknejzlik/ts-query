...
Q.select()
.from('users')
.innerJoin(Query.table('orders', 'o'), Conditions.and([
  Conditions.columnEqual('users.id', 'o.user_id'),
...
<QueryPreview
  code={`
Q.select().from('table').where(Conditions.equal('fieldName', 'foo')).where(Conditions.lessThan('fieldName2', 123));
`}
/>
...
<QueryPreview
  code={`
Q.select().from(Q.select().from('table').where(Conditions.equal('field', value)),'subt');
`}
/>
...
```tsx
Q.select().from('table').orderBy('bar', 'DESC');
