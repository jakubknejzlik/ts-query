...
```ts
const query = Query.select('tableName');
```
...
```ts
const queryWithFields = Query.select('table').field('fieldName');
```

If you need to rename a column in the result set, you can provide an alias:

```ts
const queryWithAlias = Query.select('table').field('fieldName', 'aliasName');
```

You can also add multiple fields at once:

```ts
const queryWithAlias = Query.select('table').addFields([
...
```ts
const queryWithAlias = Query.select('table')
  .field('fieldNameA')
...
```ts
const aliasedTableQuery = Query.select('table', 't');
```
...
```ts
const joinedQuery = Query.select('users').join(
...
- **Inner Join**: Combines rows from different tables when the join condition is met.
  ...
  ```ts
  const innerJoinedQuery = Query.select('users').innerJoin(
    Query.table('orders', 'o'),
    Conditions.columnEqual('users.id', 'o.user_id')
  );
  ```
...
- **Left Join**: Returns all rows from the left table, and the matched rows from the right table. If no match, NULLs are returned for columns of the right table.
  ...
  ```ts
  const leftJoinedQuery = Query.select('users').leftJoin(
    Query.table('orders', 'o'),
    Conditions.columnEqual('users.id', 'o.user_id')
  );
  ```
...
- **Right Join**: Similar to the left join, but returns all rows from the right table and matched rows from the left table.
  ...
  ```ts
  const rightJoinedQuery = Query.select('users').rightJoin(
    Query.table('orders', 'o'),
    Conditions.columnEqual('users.id', 'o.user_id')
  );
  ```
...
- **Full Join**: Combines the results of both left and right joins. It returns rows when there is a match in either left or right table.
  ...
  ```ts
  const fullJoinedQuery = Query.select('users').fullJoin(
    Query.table('orders', 'o'),
    Conditions.columnEqual('users.id', 'o.user_id')
  );
  ```
...
```ts
const multiJoinedQuery = Query.select('users')
  .innerJoin(
    Query.table('orders', 'o'),
    Conditions.columnEqual('users.id', 'o.user_id')
  )
...
```ts
const aliasedJoinQuery = Query.select('users', 'u').leftJoin(
  Query.table('orders', 'o'),
...
const complexJoinedQuery = Query.select('users').innerJoin(
  Query.table('orders', 'o'),
...
```ts
const queryWithCondition = Query.select('table').where(
  Conditions.equal('fieldName', value)
);
```
...
```ts
const orderedQuery = Query.select('table').orderBy('fieldName');
```

If you need the sorting to be in descending order:

```ts
const descOrderedQuery = Query.select('table').orderBy('fieldName', 'DESC');
```
...
```ts
const groupedQuery = Query.select('table').groupBy('fieldName');
```
...
```ts
const subQuery = Query.select('table').where(Conditions.equal('field', value));
...
```ts
const basicQuery = Query.select('foo');
...
```ts
const multiConditionQuery = Query.select('users')
  .field('id')
  .field('name')
  .where(
    Conditions.and([
...
```ts
const nestedConditionQuery = Query.select('products').where(
  Conditions.and([
...
```ts
const originalQuery = Query.select('table').orderBy('bar', 'DESC');
